                ...
                .map(|cell| {
                    ...
                    let background_color = match cell.get_style().get_fill() {
                        Some(fill) => match fill.get_pattern_fill() {
                            Some(patter_fill) => match patter_fill.get_background_color() {
                                Some(color) => "#".to_string() + &color.get_argb()[2..],
                                None => "".to_string(),
                            },
                            None => "".to_string(),
                        },
                    ...
                    CellData {
                        r: coordinate.get_row_num().to_owned(),
                        c: coordinate.get_col_num().to_owned(),
                        v: CellAttributes {
                            ct: CellFormat {
...
fn apply_ops(ops: Vec<Op>, spreadsheet_manager: State<SpreadsheetManager>) {
    ...
    for op in ops {
        ...
        if op.op == "addSheet" {
            ...
            let row = op.path[1].to_owned().as_u64().unwrap() as u32;
            let column = op.path[2].to_owned().as_u64().unwrap() as u32;
